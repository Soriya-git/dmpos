...
class DailySessionMenuController extends Controller
{
    public function __invoke(Request $request): Response
    {
        ...
        $orderLines = OrderLine::query()
            ...
            ->whereHas('menu', fn ($query) => $query->where('print_route', 'kitchen'))
            ->get();

        $rows = $orderLines
            ->groupBy('menu_id')
            ->map(fn (Collection $lines) => $this->menuRow($lines))
            ->sortBy('menuName', SORT_NATURAL | SORT_FLAG_CASE)
            ->values();

        return Inertia::render('OperationsReport/DailySessionMenu', [
            'session' => [
                'date' => $sessionDate,
    ...
    private function menuRow(Collection $lines): array
    {
        /** @var OrderLine $first */
        $first = $lines->first();
        $menu = $first->menu;
        ...
        return [
            ...
            'itemCode' => $item?->code,
            'itemName' => $item?->name,
            'unit' => $item?->unit?->code ?? $item?->unit?->name ?? 'Plate',
            'route' => 'Menu kitchen route',
    ...
    private function scope(Request $request): array
    {
        ...
        abort_if(! $companyId, 422, 'No company is available for daily session menu.');
