...
const actionInfo = computed(() => {
    ...
    if (props.putaway.status === 'cancelled') {
        return {
            status: 'Cancelled',
            name: props.putaway.cancelled_by,
            at: actionDate(props.putaway.cancelled_at),
        };
    }

    if (props.putaway.status === 'rejected') {
        return {
            status: 'Rejected',
            name: props.putaway.created_by ?? props.putaway.assigned_staff,
            at: actionDate(props.putaway.updated_at),
        };
    }
            ...
            >
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">
                        Source GR: {{ putaway.goods_receipt_no ?? '-' }}
                    </p>
                </div>
                <Button
                    type="button"
                ...
                <div class="mb-4 grid gap-3 text-sm sm:grid-cols-3">
                    <div class="space-y-1 rounded-lg bg-slate-50 p-3">
                        ...
                        <p class="font-bold text-[#2a4858]">
                            Created: {{ putaway.created_at ?? '-' }}
                        </p>
                        <p class="text-xs font-semibold text-blue-700">
                            Source: {{ putaway.goods_receipt_no ?? '-' }}
                        </p>
                    </div>
                    <div class="space-y-2 rounded-lg bg-slate-50 p-3">
                        ...
                        <p class="text-xs font-semibold text-slate-500">
                            {{ putaway.item_count }} item{{
                                putaway.item_count === 1 ? '' : 's'
                            }}
                            /
                            {{ putaway.total_quantity }} qty
                        </p>
                    </div>
                    <div class="space-y-1 rounded-lg bg-slate-50 p-3">
                        <p class="text-xs font-bold text-slate-400 uppercase">
