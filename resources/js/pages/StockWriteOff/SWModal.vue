...
const actionInfo = computed(() => {
    if (props.writeOff.actionStatus === 'Approved') {
        return {
            status: 'Approved',
            name: props.writeOff.confirmedBy,
            at: actionDate(props.writeOff.confirmedAt),
        };
    }

    if (
        props.writeOff.actionStatus === 'Rejected' ||
        props.writeOff.actionStatus === 'Cancelled'
    ) {
        return {
            status: props.writeOff.actionStatus,
            name: props.writeOff.cancelledBy,
            at: actionDate(props.writeOff.cancelledAt),
        };
    }
        ...
        >
                ...
                <div class="mb-4 grid gap-3 text-sm sm:grid-cols-3">
                    <div class="space-y-1 rounded-lg bg-slate-50 p-3">
                        ...
                        <p class="font-bold text-[#2a4858]">
                            Date: {{ writeOff.displayDate ?? '-' }}
                        </p>
                        <p class="text-xs font-semibold text-[#007882]">
                            {{ writeOff.locationCode }} /
                            {{ writeOff.location }}
                        </p>
                    </div>
                    <div class="space-y-2 rounded-lg bg-slate-50 p-3">
                        ...
                        <span
                            ...
                            :class="statusClass(writeOff.status)"
                        >
                            {{ statusLabel(writeOff.status) }}
                        </span>
                        <p class="text-xs font-semibold text-slate-500">
                            {{ writeOff.itemCount }} item{{
                                writeOff.itemCount === 1 ? '' : 's'
                            }}
                            / {{ numberValue(writeOff.totalQuantity) }} qty
                        </p>
                    </div>
                    <div class="space-y-1 rounded-lg bg-slate-50 p-3">
                        <p class="text-xs font-bold text-slate-400 uppercase">
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                >
                                    {{ numberValue(line.writeOffQuantity) }}
                                </td>
                                <td
            ...
            >
                <div class="flex items-center gap-2 text-sm text-slate-500">
                    ...
                    {{ writeOff.lines.length }} item line{{
                        writeOff.lines.length === 1 ? '' : 's'
                    }}
                </div>
